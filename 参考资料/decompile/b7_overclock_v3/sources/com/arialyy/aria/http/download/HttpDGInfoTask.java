...
public final class HttpDGInfoTask implements IInfoTask {
    ...
    private IInfoTask.Callback subCallback = new IInfoTask.Callback() { // from class: com.arialyy.aria.http.download.HttpDGInfoTask.1
        ...
        public void onFail(AbsEntity absEntity, AriaException ariaException, boolean z10) {
            ...
            ((DGInfoCallback) HttpDGInfoTask.this.callback).onSubFail(downloadEntity, new AriaHTTPException(String.format("子任务获取文件长度失败，url：%s", downloadEntity.getUrl())), z10);
    ...
    public interface DGInfoCallback extends IInfoTask.Callback {
        ...
        void onSubFail(DownloadEntity downloadEntity, AriaHTTPException ariaHTTPException, boolean z10);
    }

    public HttpDGInfoTask(DGTaskWrapper dGTaskWrapper) {
        this.wrapper = dGTaskWrapper;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* JADX WARN: Multi-variable type inference failed */
    public void checkGetSizeComplete(int i10, int i11) {
        if (this.isStop || this.isCancel) {
            ALog.w(this.TAG, "任务已停止或已取消，isStop = " + this.isStop + ", isCancel = " + this.isCancel);
            notifyLock();
            return;
        }
        if (i11 == this.wrapper.getSubTaskWrapper().size()) {
            this.callback.onFail(this.wrapper.getEntity(), new AriaHTTPException("获取子任务长度失败"), false);
            notifyLock();
            return;
        }
        if (i10 == this.wrapper.getSubTaskWrapper().size()) {
            long fileSize = 0;
            Iterator<DTaskWrapper> it2 = this.wrapper.getSubTaskWrapper().iterator();
            while (it2.hasNext()) {
                fileSize += it2.next().getEntity().getFileSize();
            }
            ...
            ALog.d(this.TAG, String.format("获取组合任务长度完成，组合任务总长度：%s，失败的子任务数：%s", Long.valueOf(fileSize), Integer.valueOf(i11)));
