...
final class SqlHelper extends SQLiteOpenHelper {
    ...
    private void addTaskRecordType(SQLiteDatabase sQLiteDatabase) {
        try {
            ...
            while (true) {
                ...
                if (!string2.startsWith("ftp") && !string2.startsWith("sftp")) {
                    if (SqlUtil.tableExists(sQLiteDatabase, (Class<? extends DbEntity>) M3U8Entity.class)) {
                        ...
                        if (cursorRawQuery2.moveToNext()) {
                            ...
                            i10 = (!TextUtils.isEmpty(string3) && Boolean.valueOf(string3).booleanValue()) ? 8 : 7;
                        } else {
                            i10 = 1;
                        }
                ...
                sQLiteDatabase.execSQL("UPDATE DownloadEntity SET taskType=? WHERE downloadPath=?", new Object[]{Integer.valueOf(i10), string});
                sQLiteDatabase.execSQL("UPDATE TaskRecord SET taskType=? WHERE filePath=?", new Object[]{Integer.valueOf(i10), string});
                sQLiteDatabase.execSQL("UPDATE ThreadRecord SET threadType=? WHERE taskKey=?", new Object[]{Integer.valueOf(i10), string});
            }
            ...
            while (cursorRawQuery3.moveToNext()) {
                String string4 = cursorRawQuery3.getString(cursorRawQuery3.getColumnIndex("filePath"));
                String string5 = cursorRawQuery3.getString(cursorRawQuery3.getColumnIndex("url"));
                int i11 = (string5.startsWith("ftp") || string5.startsWith("sftp")) ? 3 : 1;
                sQLiteDatabase.execSQL("UPDATE UploadEntity SET taskType=? WHERE filePath=?", new Object[]{Integer.valueOf(i11), string4});
                sQLiteDatabase.execSQL("UPDATE TaskRecord SET taskType=? WHERE filePath=?", new Object[]{Integer.valueOf(i11), string4});
                sQLiteDatabase.execSQL("UPDATE ThreadRecord SET threadType=? WHERE taskKey=?", new Object[]{Integer.valueOf(i11), string4});
            }
                ...
                for (String str : DBConfig.mapping.keySet()) {
                    ...
                    if (SqlUtil.tableExists(sQLiteDatabase, cls)) {
                        ...
                        if (map != null) {
                            map2 = map.get(str);
                            if (map2 != null) {
                                Iterator<String> it2 = newColumn.iterator();
                                while (it2.hasNext()) {
    ...
    public void onCreate(SQLiteDatabase sQLiteDatabase) {
        ...
        while (it2.hasNext()) {
            Class<? extends DbEntity> cls = DBConfig.mapping.get(it2.next());
            if (!SqlUtil.tableExists(sQLiteDatabase, cls)) {
                SqlUtil.createTable(sQLiteDatabase, cls);
            }
        }
