...
abstract class AbsGroupConfigHandler<TARGET extends AbsTarget> implements IConfigHandler {
    ...
    public AbsGroupConfigHandler(TARGET target, long j10) {
        this.mTarget = target;
        DGTaskWrapper dGTaskWrapper = (DGTaskWrapper) TaskWrapperManager.getInstance().getGroupWrapper(DGTaskWrapper.class, j10);
        this.mWrapper = dGTaskWrapper;
        if (this.mTarget instanceof AbsNormalTarget) {
            if (j10 < 0) {
                dGTaskWrapper.setErrorEvent(new ErrorEvent(j10, "任务id为空"));
