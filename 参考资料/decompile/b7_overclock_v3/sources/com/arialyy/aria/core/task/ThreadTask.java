...
public class ThreadTask implements IThreadTask, IThreadTaskObserver {
    ...
    public ThreadTask(SubThreadConfig subThreadConfig) {
        ...
        long j10 = this.mRecord.startLocation;
        this.mRangeProgress = j10;
        this.mLastRangeProgress = j10;
    ...
    private void handleBlockRecord() {
        ...
        if (subThreadConfig.isBlock) {
            ...
            long j10 = this.mRecord.blockLen;
            if (!file.exists()) {
                ...
                threadRecord.startLocation = threadRecord.endLocation - j10;
            } else if (length > j10) {
                ...
                long j11 = threadRecord3.blockLen;
                if (length < j11) {
                    threadRecord3.startLocation = (threadRecord3.endLocation - j11) + length;
    ...
    private void retryBlockTask(boolean z10) {
        ...
        if (this.mFailTimes >= 2 || !z10 || (!(NetUtils.isConnected(AriaConfig.getInstance().getAPP()) || this.isNotNetRetry) || isBreak())) {
            ALog.e(this.TAG, String.format("重试分开【%s】失败，failTimes = %s, needRetry = %s, isBreak = %s", getFileName(), Integer.valueOf(this.mFailTimes), Boolean.valueOf(z10), Boolean.valueOf(isBreak())));
            sendFailMsg(null, z10);
        } else {
    ...
    private void retryM3U8Peer(boolean z10) {
    ...
    private void sendFailMsg(AriaException ariaException, boolean z10) {
        Bundle bundle = new Bundle();
        bundle.putBoolean(IThreadStateManager.DATA_RETRY, z10);
    ...
    private void sendM3U8Info(int i10, Message message) {
    ...
    public void writeConfig(boolean z10, long j10) {
        ...
        if (threadRecord != null) {
            ...
            } else if (0 < j10 && j10 < threadRecord.endLocation) {
    ...
    public void breakTask() {
        ...
        long j10 = this.mRangeProgress;
        updateState(1, null);
        ALog.d(this.TAG, String.format("任务【%s】thread__%s__中断【停止位置：%s】", getFileName(), Integer.valueOf(this.mRecord.threadId), Long.valueOf(j10)));
    ...
    public void fail(long j10, AriaException ariaException, boolean z10) throws Throwable {
        if (ariaException != null) {
    ...
    public void setMaxSpeed(int i10) {
        ...
        if (iThreadTaskAdapter != null) {
            iThreadTaskAdapter.setMaxSpeed(i10);
        }
    }
    ...
    public void stop() {
        ...
        if (this.mTaskWrapper.getRequestType() == 7) {
            ...
            ALog.d(this.TAG, String.format("任务【%s】thread__%s__停止【当前线程停止位置：%s】", getFileName(), Integer.valueOf(this.mRecord.threadId), Long.valueOf(j10)));
            writeConfig(false, j10);
        }
    }
    ...
    @Override // com.arialyy.aria.core.task.IThreadTaskObserver
    public synchronized void updateFailState(AriaException ariaException, boolean z10) {
        fail(this.mRangeProgress, ariaException, z10);
    }
    ...
    @Override // com.arialyy.aria.core.task.IThreadTaskObserver
    public synchronized void updateState(int i10, Bundle bundle) {
        Message messageObtainMessage = this.mStateHandler.obtainMessage();
        if (bundle == null) {
            bundle = new Bundle();
        }
        ...
        if (getConfig().threadType == 3) {
            sendM3U8Info(i10, messageObtainMessage);
        }
        Thread thread = this.mStateHandler.getLooper().getThread();
        if (thread.isAlive() && !thread.isInterrupted()) {
            messageObtainMessage.sendToTarget();
