...
class DGTaskWrapperFactory implements IGroupWrapperFactory<DownloadGroupEntity, DGTaskWrapper> {
    ...
    private DownloadGroupEntity getOrCreateHttpDGEntity(long j10) {
    ...
    public DGTaskWrapper getGroupWrapper(long j10) {
        DGTaskWrapper dGTaskWrapper;
        if (j10 == -1) {
            dGTaskWrapper = new DGTaskWrapper(new DownloadGroupEntity());
        } else {
            DownloadGroupEntity orCreateHttpDGEntity = getOrCreateHttpDGEntity(j10);
