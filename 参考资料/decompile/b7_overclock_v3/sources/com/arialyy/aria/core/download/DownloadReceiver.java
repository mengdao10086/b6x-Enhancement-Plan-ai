...
public class DownloadReceiver extends AbsReceiver {
    ...
    public DownloadEntity getDownloadEntity(long j10) {
        if (j10 >= 0) {
            return (DownloadEntity) DbEntity.findFirst(DownloadEntity.class, "rowid=?", String.valueOf(j10));
        }
    ...
    public DownloadGroupEntity getGroupEntity(long j10) {
        ...
        return DbDataHelper.getDGEntity(j10);
    }
    ...
    @Deprecated
    public DownloadReceiver setMaxSpeed(int i10) throws Throwable {
        AriaConfig.getInstance().getDConfig().setMaxSpeed(i10);
        return this;
    }
    ...
    public List<DownloadEntity> getAllCompleteTask(int i10, int i11) {
        CheckUtil.checkPageParams(i10, i11);
        return DbEntity.findDatas(DownloadEntity.class, "isGroupChild=? and downloadPath!='' and isComplete=?", "false", "true");
    }

    public List<DownloadEntity> getAllNotCompleteTask(int i10, int i11) {
        CheckUtil.checkPageParams(i10, i11);
        return DbEntity.findDatas(DownloadEntity.class, i10, i11, "isGroupChild=? and downloadPath!='' and isComplete=?", "false", "false");
    }

    public List<DownloadGroupEntity> getGroupTaskList(int i10, int i11) {
        List listFindRelationData = DbEntity.findRelationData(DGEntityWrapper.class, i10, i11, new String[0]);
        if (listFindRelationData == null || listFindRelationData.isEmpty()) {
            return null;
        }
        ArrayList arrayList = new ArrayList();
        Iterator it2 = listFindRelationData.iterator();
        while (it2.hasNext()) {
    ...
    public List<DownloadEntity> getTaskList(int i10, int i11) {
        CheckUtil.checkPageParams(i10, i11);
        return DbEntity.findDatas(DownloadEntity.class, i10, i11, "isGroupChild=? and downloadPath!=''", "false");
    }
    ...
    public FtpDirNormalTarget loadFtpDir(long j10) {
        ComponentUtil.getInstance().checkComponentExist(2);
        return (FtpDirNormalTarget) DTargetFactory.getInstance().generateNormalTarget(FtpDirNormalTarget.class, j10);
    }

    public GroupNormalTarget loadGroup(long j10) {
        ComponentUtil.getInstance().checkComponentExist(1);
        return (GroupNormalTarget) DTargetFactory.getInstance().generateNormalTarget(GroupNormalTarget.class, j10);
    }

    public HttpNormalTarget load(long j10) {
        ComponentUtil.getInstance().checkComponentExist(1);
        return (HttpNormalTarget) DTargetFactory.getInstance().generateNormalTarget(HttpNormalTarget.class, j10);
    }

    public FtpNormalTarget loadFtp(long j10) {
        ComponentUtil.getInstance().checkComponentExist(2);
        return (FtpNormalTarget) DTargetFactory.getInstance().generateNormalTarget(FtpNormalTarget.class, j10);
    }
}
