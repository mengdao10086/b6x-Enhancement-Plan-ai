...
class UNormalConfigHandler<TARGET extends AbsTarget> implements IConfigHandler {
    ...
    public UNormalConfigHandler(TARGET target, long j10) {
        this.mTarget = target;
        initTarget(j10);
    }
    ...
    private void initTarget(long j10) {
        UTaskWrapper uTaskWrapper = (UTaskWrapper) TaskWrapperManager.getInstance().getNormalTaskWrapper(UTaskWrapper.class, j10);
        this.mWrapper = uTaskWrapper;
        if (this.mTarget instanceof AbsNormalTarget) {
            if (j10 < 0) {
                uTaskWrapper.setErrorEvent(new ErrorEvent(j10, "任务id为空"));
            } else if (uTaskWrapper.getEntity().getId() < 0) {
                this.mWrapper.setErrorEvent(new ErrorEvent(j10, "任务信息不存在"));
            }
        }
