...
public class TaskSchedulers<TASK extends ITask> implements ISchedulers {
    ...
    private ISchedulerListener createListener(String str) {
        try {
            ...
            ALog.e(this.TAG, e10.getMessage());
            return null;
        } catch (IllegalAccessException e11) {
            ALog.e(this.TAG, e11.getMessage());
            return null;
        } catch (InstantiationException e12) {
            ALog.e(this.TAG, e12.getMessage());
            return null;
        }
    }
    ...
    private void handleNormalEvent(TASK task, int i10) {
    ...
    private boolean handlePeerEvent(Message message) {
        ...
        if (this.mObservers.size() > 0) {
            ...
            while (it2.hasNext()) {
                Map<TaskEnum, Object> map = this.mObservers.get(it2.next());
    ...
    */
    ...
    private void normalTaskCallback(int i10, TASK task) {
            ...
            for (String str : this.mObservers.keySet()) {
                ...
                if (map != null && !map.isEmpty()) {
                    ...
                    if (normalTaskListenerInterface != null) {
                        normalTaskCallback(i10, task, normalTaskListenerInterface);
                    }
                }
            }
        }
    ...
    private void sendNormalBroadcast(int i10, TASK task) {
        if (this.mAriaConfig.getAConfig().isUseBroadcast()) {
            ...
            if (taskType == 1 || taskType == 3) {
                this.mAriaConfig.getAPP().sendBroadcast(createData(i10, taskType, task.getTaskWrapper().getEntity()));
            } else if (taskType == 2) {
                this.mAriaConfig.getAPP().sendBroadcast(createData(i10, taskType, task.getTaskWrapper().getEntity()));
            } else {
    ...
    public ITaskQueue getQueue(int i10) {
        if (i10 == 1) {
        ...
        throw new NullPointerException("任务类型错误，type = " + i10);
    }
    ...
    public void startNextTask(ITaskQueue iTaskQueue, int i10) {
        if (i10 == 2) {
            return;
        }
    ...
    public void unRegister(Object obj) {
        if (this.mObservers.containsKey(getKey(obj))) {
            Iterator<Map.Entry<String, Map<TaskEnum, Object>>> it2 = this.mObservers.entrySet().iterator();
            while (it2.hasNext()) {
                if (it2.next().getKey().equals(getKey(obj))) {
                    it2.remove();
                }
            }
        }
    ...
    private void normalTaskCallback(int i10, TASK task, NormalTaskListenerInterface<TASK> normalTaskListenerInterface) {
        if (normalTaskListenerInterface != null) {
            if (task == null && i10 != 11) {
