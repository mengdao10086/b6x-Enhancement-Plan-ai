...
class DNormalConfigHandler<TARGET extends AbsTarget> implements IConfigHandler {
    ...
    private void initTarget(long j10) {
        DTaskWrapper dTaskWrapper = (DTaskWrapper) TaskWrapperManager.getInstance().getNormalTaskWrapper(DTaskWrapper.class, j10);
        this.mWrapper = dTaskWrapper;
        if (this.mTarget instanceof AbsNormalTarget) {
            if (j10 < 0) {
                dTaskWrapper.setErrorEvent(new ErrorEvent(j10, "任务id为空"));
