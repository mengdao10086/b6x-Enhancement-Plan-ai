...
public class DTargetFactory {
    ...
    public <T extends AbsNormalTarget> T generateNormalTarget(Class<T> cls, long j10) {
        if (cls == HttpNormalTarget.class) {
            return new HttpNormalTarget(j10);
        }
        if (cls == FtpNormalTarget.class) {
            return new FtpNormalTarget(j10);
        }
        if (cls == GroupNormalTarget.class) {
            return new GroupNormalTarget(j10);
        }
        if (cls == FtpDirNormalTarget.class) {
            return new FtpDirNormalTarget(j10);
        }
        return null;
    }
}
