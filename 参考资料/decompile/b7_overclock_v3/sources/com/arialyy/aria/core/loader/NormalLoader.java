...
public class NormalLoader<T extends AbsTaskWrapper> extends AbsNormalLoader<T> {
    ...
    public NormalLoader(T t10, IEventListener iEventListener) {
        super(t10, iEventListener);
        this.isComplete = false;
        this.mTempFile = new File(getEntity().getFilePath());
        EventMsgUtil.getDefault().register(this);
        setUpdateInterval(t10.getConfig().getUpdateInterval());
    }
    ...
    public void setMaxSpeed(int i10) {
        ...
        for (IThreadTask iThreadTask : getTaskList()) {
            if (iThreadTask != null && (i11 = this.startThreadNum) > 0) {
                iThreadTask.setMaxSpeed(i10 / i11);
            }
        }
    }
    ...
    public void startThreadTask() {
        ...
        Iterator<IThreadTask> it2 = getTaskList().iterator();
        while (it2.hasNext()) {
            ThreadTaskManager.getInstance().startThread(this.mTaskWrapper.getKey(), it2.next());
        }
        startTimer();
    }
    ...
    public void addComponent(IInfoTask iInfoTask) {
        ...
        iInfoTask.setCallback(new IInfoTask.Callback() { // from class: com.arialyy.aria.core.loader.NormalLoader.1
            ...
            public void onFail(AbsEntity absEntity, AriaException ariaException, boolean z10) {
                NormalLoader.this.getListener().onFail(z10, ariaException);
            }
