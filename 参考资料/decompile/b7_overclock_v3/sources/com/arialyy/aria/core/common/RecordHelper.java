...
public class RecordHelper {
    ...
    public void handleBlockRecord() {
        ...
        for (ThreadRecord threadRecord : this.mTaskRecord.threadRecords) {
            long j10 = threadRecord.blockLen;
            ...
            if (!file.exists()) {
                ...
                ALog.i(this.TAG, String.format("startLocation = %s; endLocation = %s; block = %s; tempLen = %s; threadId = %s", Long.valueOf(threadRecord.startLocation), Long.valueOf(threadRecord.endLocation), Long.valueOf(j10), Long.valueOf(file.length()), Integer.valueOf(threadRecord.threadId)));
                long length = file.length();
                if (length > j10) {
                    ALog.i(this.TAG, String.format("分块【%s】错误，分块长度【%s】 > 线程区间长度【%s】，将重新开始该分块", Integer.valueOf(threadRecord.threadId), Long.valueOf(length), Long.valueOf(j10)));
                    file.delete();
                    threadRecord.startLocation = ((long) threadRecord.threadId) * j10;
                } else {
                    long j11 = (((long) threadRecord.threadId) * fileSize) + length;
                    ...
                    } else if (threadRecord.startLocation != j11) {
                        ALog.i(this.TAG, String.format("修正分块【%s】的进度记录为：%s", file.getPath(), Long.valueOf(j11)));
                        threadRecord.startLocation = j11;
                    } else {
                        ALog.i(this.TAG, String.format("修正分块【%s】的进度记录为：%s", file.getPath(), Long.valueOf(j11)));
                        threadRecord.startLocation = j11;
                        threadRecord.isComplete = false;
    ...
    public void handleMultiRecord() {
        ...
        while (true) {
            ...
            long j10 = ((long) i10) * fileSize;
            int i11 = i10 + 1;
            long fileSize2 = ((long) i11) * fileSize;
            ThreadRecord threadRecord = taskRecord.threadRecords.get(i10);
