...
public class ResumeThread implements Runnable {
    ...
    public ResumeThread(boolean z10, String str) {
    ...
    private void findTaskData(int i10) {
        List listFindDatas;
        if (i10 == 1) {
            ...
            Iterator it2 = listFindDatas2.iterator();
            while (it2.hasNext()) {
                addResumeEntity(TaskWrapperManager.getInstance().getNormalTaskWrapper(DTaskWrapper.class, ((DownloadEntity) it2.next()).getId()));
            }
            return;
        }
        if (i10 == 2) {
            ...
            Iterator it3 = listFindDatas3.iterator();
            while (it3.hasNext()) {
                addResumeEntity(TaskWrapperManager.getInstance().getGroupWrapper(DGTaskWrapper.class, ((DownloadGroupEntity) it3.next()).getId()));
            }
            return;
        }
        if (i10 != 3 || (listFindDatas = DbEntity.findDatas(UploadEntity.class, String.format("NOT(isComplete) AND %s ORDER BY stopTime DESC", this.sqlCondition))) == null || listFindDatas.isEmpty()) {
            return;
        }
        Iterator it4 = listFindDatas.iterator();
        while (it4.hasNext()) {
            addResumeEntity(TaskWrapperManager.getInstance().getNormalTaskWrapper(UTaskWrapper.class, ((UploadEntity) it4.next()).getId()));
        }
    }
