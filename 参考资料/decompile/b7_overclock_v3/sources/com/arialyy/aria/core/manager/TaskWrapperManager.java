...
public class TaskWrapperManager {
    ...
    private String convertKey(Class cls, long j10) {
        return CommonUtil.keyToHashKey(cls.getName() + j10);
    }
    ...
    public <TW extends AbsTaskWrapper> TW getGroupWrapper(Class<TW> cls, long j10) {
        Lock lock = this.lock;
        lock.lock();
        try {
            TW tw = (TW) this.cache.get(convertKey(cls, j10));
            if (tw == null || tw.getClass() != cls) {
                ...
                tw = (TW) iGroupWrapperFactoryChooseGroupFactory.getGroupWrapper(j10);
                putTaskWrapper(tw);
            }
            return tw;
        } finally {
    ...
    public <TW extends AbsTaskWrapper> TW getNormalTaskWrapper(Class<TW> cls, long j10) {
        Lock lock = this.lock;
        lock.lock();
        try {
            TW tw = (TW) this.cache.get(convertKey(cls, j10));
            if (tw == null || tw.getClass() != cls) {
                ...
                tw = (TW) iNormalTEFactoryChooseNormalFactory.create(j10);
                putTaskWrapper(tw);
            }
            return tw;
        } finally {
