...
public final class TreeRangeMap<K extends Comparable, V> implements RangeMap<K, V> {
    ...
    public static final class RangeMapEntry<K extends Comparable, V> extends AbstractMapEntry<Range<K>, V> {
        ...
        public RangeMapEntry(Cut<K> cut, Cut<K> cut2, V v10) {
            this(Range.create(cut, cut2), v10);
        }

        public boolean contains(K k10) {
            return this.range.contains(k10);
        }
    ...
    public class SubRangeMap implements RangeMap<K, V> {
        ...
        public class SubRangeMapAsMap extends AbstractMap<Range<K>, V> {
            ...
            public Iterator<Map.Entry<Range<K>, V>> entryIterator() {
                ...
                final Iterator<V> it2 = TreeRangeMap.this.entriesByLowerBound.tailMap((Cut) MoreObjects.firstNonNull(TreeRangeMap.this.entriesByLowerBound.floorKey(SubRangeMap.this.subRange.lowerBound), SubRangeMap.this.subRange.lowerBound), true).values().iterator();
        ...
        public Map<Range<K>, V> asDescendingMapOfRanges() {
            return new TreeRangeMap<K, V>.SubRangeMap.SubRangeMapAsMap() { // from class: com.google.common.collect.TreeRangeMap.SubRangeMap.1
                ...
                public Iterator<Map.Entry<Range<K>, V>> entryIterator() {
                    ...
                    final Iterator<V> it2 = TreeRangeMap.this.entriesByLowerBound.headMap(SubRangeMap.this.subRange.upperBound, false).descendingMap().values().iterator();
        ...
        public void putCoalescing(Range<K> range, V v10) {
            if (TreeRangeMap.this.entriesByLowerBound.isEmpty() || range.isEmpty() || !this.subRange.encloses(range)) {
                ...
                put(TreeRangeMap.this.coalescedRange(range, Preconditions.checkNotNull(v10)).intersection(this.subRange), v10);
            }
        }
    ...
    private static <K extends Comparable, V> Range<K> coalesce(Range<K> range, V v10, @NullableDecl Map.Entry<Cut<K>, RangeMapEntry<K, V>> entry) {
        return (entry != null && entry.getValue().getKey().isConnected(range) && entry.getValue().getValue().equals(v10)) ? (Range<K>) range.span(entry.getValue().getKey()) : range;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* JADX WARN: Type inference incomplete: some casts might be missing */
    public Range<K> coalescedRange(Range<K> range, V v10) {
        return coalesce(coalesce(range, v10, this.entriesByLowerBound.lowerEntry((Cut<K>) range.lowerBound)), v10, this.entriesByLowerBound.floorEntry((Cut<K>) range.upperBound));
    }
    ...
    @NullableDecl
    public Map.Entry<Range<K>, V> getEntry(K k10) {
        Map.Entry<Cut<K>, RangeMapEntry<K, V>> entryFloorEntry = this.entriesByLowerBound.floorEntry(Cut.belowValue(k10));
        if (entryFloorEntry == null || !entryFloorEntry.getValue().contains(k10)) {
            return null;
        }
