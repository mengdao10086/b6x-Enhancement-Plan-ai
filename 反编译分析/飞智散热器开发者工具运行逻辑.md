...
> **说明**：本文档基于 `../参考资料/decompile/b6_devtool/smali_merged/` 的反编译输出，梳理 App（`com.flydigi.waspwing.experimental`）的核心运行逻辑。主要面向需要理解 App 内部工作原理以维护 LSPosed 模块的场景。
>
> 📦 对应 APK：`../参考资料/apk_original/飞智散热器开发者工具.apk`
> 🔧 涉及模块引用：`lsp模块/app/src/main/java/.../MainHook.java`
> 📖 修复历程：`../参考资料/完整修复历程.md`
...
2. **BLE 状态写入线程启动**：模块的 `startPeriodicStatusWrite()` 在 `Application.onCreate` 的 after-hook 中启动（[MainHook.java](../lsp模块/app/src/main/java/com/example/waspwingtempctrl/MainHook.java)，按方法名搜索定位，行号随代码重构变化），每 1 秒写入 status 文件供 tempctrl 守护进程检测心跳。
...
结果：UI 显示已连接，但扫描动画永不停止。此 Bug 由 LSPosed 模块修复（钩 `AbstractBluetoothController.onDeviceConnected` → `stopScan()` + 钩 `BluetoothViewModel.onDeviceConnected` → 更新 LiveData，见 [MainHook.java](../lsp模块/app/src/main/java/com/example/waspwingtempctrl/MainHook.java)）解决。
...
> 🔗 LSPosed 钩子位置：`LeDataInteractionController.checkBluetoothPermission` → [MainHook.java](../lsp模块/app/src/main/java/com/example/waspwingtempctrl/MainHook.java) 中按方法名搜索定位（强制返回 true）
...
> 🔗 诊断钩子位置（`handleLoadPackage` 中统一注册，行号随代码重构变化，按方法名搜索定位）：
> - `discoverServices`
> - `connect`
> - `onServicesDiscovered`
> - `onConnectionStateChange`
> - `SDK.onGattConnected`
> - `SDK.onDeviceInfoUpdate`
...
三个问题叠加导致 Android 16 上 App 完全无法使用散热器。目前已全部通过 LSPosed 模块修复（详细修复记录见 [完整修复历程.md](../参考资料/完整修复历程.md)）。
...
### 5.3 锁死现象（制冷 125 / 风扇 4500）与命令丢失链路

**现象**：散热器偶发锁死在制冷 125、风扇 4500，tempctrl 持续下发新命令但设备不响应；常在 BLE 重连后出现（多次快速重连后更易触发），偶发无重连也出现。

**关键证据**（lsp 日志 + 反编译）：
- 4500/125 从未被 tempctrl 下发过，是设备固件在"未收到有效控制命令"时的默认固定功率档（`coldLevelOverclock=byte[0xd]=125`、`windLevelOverclock=byte[0xc]×100+byte[0x13]=45×100`）。
- 单次连接可能触发**多个 controller 实例**同时 `onGattConnected`（日志可见同一时刻 6 次"static 同步"），static `dataInteractionController` 指向"最后连接成功"的实例，可能并非正在接收 0x13 状态包的实例。
- SDK 有**两处命令静默丢弃点**：
  1. `AbstractDataInteractionController.processData`：`mDataConnectState != 2` 时命令出队即丢弃（"Gatt hasn't connected"）；
  2. `LeDataInteractionController.writeToBluetoothDevice`：只检查 `mBluetoothGatt != null`，**不检查是否已连接**——gatt 失效时 `writeCharacteristic` 静默返回 false，命令丢失。

**根因**：重连时 SDK 创建多 controller 实例 → static 指向的实例 gatt 失效/是旧实例（但 state 仍=2）→ setRunMode 命令通过 state 检查、写入失效 gatt 被丢弃 → 设备停在固件默认档 4500/125 → tempctrl 读回传持续下发 → 全部丢弃 → 锁死。

**已实施修复**（LSP 模块）：下发前校验 static controller 的 gatt 是否等于当前有效连接（`currentValidGatt`），失效时从已连接实例注册表重新同步；连续 3 次参数下发后回传仍停滞且 ≠ 下发目标时自动强制重连（10s 防抖）。

---

> **数据来源**：`../参考资料/decompile/b6_devtool/smali_merged/` 反编译输出 + 运行时日志分析 + LSPosed 模块钩子调试
