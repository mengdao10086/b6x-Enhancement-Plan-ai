...
2. **BLE 状态写入线程启动**：模块的 `startPeriodicStatusWrite()` 在 `Application.onCreate` 的 after-hook 中启动（[MainHook.java](../../lsp模块(apk修复+温控接口)/app/src/main/java/com/example/waspwingtempctrl/MainHook.java)，按方法名搜索定位，行号随代码重构变化），每 1 秒写入 status 文件供 tempctrl 守护进程检测心跳。
...
结果：UI 显示已连接，但扫描动画永不停止。此 Bug 由 LSPosed 模块修复（钩 `AbstractBluetoothController.onDeviceConnected` → `stopScan()` + 钩 `BluetoothViewModel.onDeviceConnected` → 更新 LiveData，见 [MainHook.java](../../lsp模块(apk修复+温控接口)/app/src/main/java/com/example/waspwingtempctrl/MainHook.java)）解决。
...
> 🔗 LSPosed 钩子位置：`LeDataInteractionController.checkBluetoothPermission` → [MainHook.java](../../lsp模块(apk修复+温控接口)/app/src/main/java/com/example/waspwingtempctrl/MainHook.java) 中按方法名搜索定位（强制返回 true）
