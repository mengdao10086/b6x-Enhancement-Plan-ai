...
window.B6X_SCHEMA = {
  ...
  keys: {
    // ---- [0] ----
    PERF_ENABLED: { type: "switch", label: "性能参数总开关",
      desc: "开关驱动折叠：关闭时性能分区折叠；可手动展开查看/编辑。=1 才加载全部性能参数" },

    // ---- [1] ----
    DEBUG_ENABLED: { type: "switch", label: "调试日志总开关",
      desc: "关闭时下方子开关折叠；可手动展开查看/编辑。=1 开启调试分区输出" },
    DEBUG_SENSOR: { type: "switch", label: "传感器读数", desc: "" },
    DEBUG_EMERG:  { type: "switch", label: "紧急干预", desc: "" },
    DEBUG_BATT:   { type: "switch", label: "电池温度控制", desc: "" },
    ...
    BATT_TEMP_DIVISOR: { type: "int", min: 1, max: 10000, label: "电池温度缩放系数",
      desc: "原始值 ÷ 缩放系数 → 0.1°C" },
    BATT_CURRENT_PATH: { type: "path", label: "电池电流 sysfs", desc: "" },
    ...
    RATE_LIMIT_FAN_DOWN: { type: "int", min: 50, max: 2000, label: "风扇降速限制", desc: "" },
    ...
    RATE_LIMIT_TEMP: { type: "int", min: 1, max: 30, label: "目标温度限速（°C）", desc: "" },
    CTRL_MODE: { type: "switch", label: "控制模式", desc: "0=常规 Gear 模式；1=PID 无级调节" },
    ...
    CURRENT_GEAR_SMOOTH_ALPHA: { type: "int", min: 1, max: 100, label: "电流滤波强度（%）", desc: "" },
