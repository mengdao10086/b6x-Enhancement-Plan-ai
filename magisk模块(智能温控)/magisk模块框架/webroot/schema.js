...
window.B6X_SCHEMA = {
    ...
    // 不控制折叠；展开/折叠由 PERF_ENABLED 总开关驱动（master）。组内按模式切换显示参数面板
    {
      ...
      modePanels: [
        { when: "1", title: "PID 模式控制",
          keys: ["PID_KP", "PID_KI", "PID_KI_VAR", "PID_KI_DEADBAND", "PID_KD",
                 "PID_INPUT_FILTER_ENABLED", "PID_FILTER_AUTO_THRESHOLD", "PID_ALPHA",
                 "PID_CPU_COMP_ENABLED", "PID_CPU_COMP_DIVISOR", "PID_PREDICT_WEIGHT",
                 "PID_PREDICT_WIN", "PID_PREDICT_RISE", "PID_COLD"] },
  ...
  keys: {
    ...
    PID_KI: { type: "multi", fields: [{ label: "积分增益", min: 0, max: 1000 }, { label: "积分上限", min: 0, max: 1000 }],
      label: "KI 积分（增益 + 上限）",
      desc: "增益：每周期每度偏差每KI为0.001归一化输出（0.19制冷）；上限：I项最大输出防饱和" },
    PID_KI_VAR: { type: "multi", fields: [{ label: "方差阈值", min: 0, max: 200 }, { label: "采样数", min: 2, max: 20 }],
      label: "KI 方差门控（阈值 + 采样数）",
      desc: "阈值：温度方差低于此值则 I 项额外启用，0=退回仅死区模式；采样数：越多越滤波但响应越慢" },
    PID_KI_DEADBAND: { type: "int", min: 0, max: 100, label: "KI 积分分离死区（0.1°C）",
      desc: "温度偏差 < 此值则启用 I 项" },
    PID_KD: { type: "int", min: 0, max: 1000, label: "KD 微分",
      desc: "抑制过冲，每周期每度变化每KD为0.001归一化输出（0.19制冷）" },
    ...
    PID_CPU_COMP_DIVISOR: { type: "int", min: 5, max: 200, label: "CPU 补偿除数",
      desc: "comp = clamp(cpu-batt,0)÷divisor→0.1°C（范围 5~200）" },
    ...
    PID_PREDICT_RISE: { type: "multi", fields: [{ label: "最大温升(0.1°C)", min: 10, max: 100 }, { label: "Ramp-up 周期数", min: 0, max: 10 }, { label: "最小起始delta(0.1°C/周期)", min: 1, max: 10 }],
