...
window.B6X_SCHEMA = {
  ...
  keys: {
    ...
    BATT_BASELINE: { type: "int", min: 300, max: 500, label: "基准温度（0.1°C）",
      desc: "" },
    CPU_FILTER_ALPHA: { type: "int", min: 1, max: 100, label: "CPU 温度滤波系数(%)",
      desc: "CPU 温度 EMA 平滑系数；首次直取，此后按此系数平滑，越大越跟随原始值" },
    ...
    PID_CH_THRESHOLD: { type: "int", min: 1, max: 100, label: "稳态冻结阈值(0.1°C)",
      desc: "温度未变且上次 |ch|≤此值/10 时整轮冻结（防抖）；默认 2=0.2°C" },
    ...
    WEBUI_LABEL_MERGE_PX: { type: "int", min: 1, max: 40, label: "标注合并阈值（px）",
      desc: "曲线头部垂直相距不超过此值且上下端都落入才合并为一行；默认 9=0.8×标签高" }
  },
