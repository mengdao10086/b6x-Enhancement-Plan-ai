...
window.B6X_SCHEMA = {
    ...
    // 不控制折叠；展开/折叠由 PERF_ENABLED 总开关驱动（master）。组内按模式切换显示参数面板
    {
      ...
      modePanels: [
        { when: "1", title: "PID 模式控制",
          keys: ["PID_KP", "PID_KI", "PID_KI_MEM", "PID_KI_FALL", "PID_KI_NEAR", "PID_KI_VAR",
  ...
  keys: {
    ...
    PID_KI_NEAR: { type: "multi", fields: [{ label: "基准速率(×0.1)", min: 0, max: 50 }, { label: "缩放", min: 10, max: 1000 }, { label: "窗口重算数", min: 2, max: 4 }, { label: "窗口周期数", min: 2, max: 8 }],
      label: "KI 近基准速度衰减",
      desc: "基准速率 / 缩放 / 窗口重算数 / 窗口周期数；近基准时按温度变化速度调衰减（降温快→加快、升温快→减慢释放）" },
    ...
    PID_KD_NEAR: { type: "multi", fields: [{ label: "近区阈值(0.1°C)", min: 0, max: 100 }, { label: "近区倍率(%)", min: 1, max: 100 }],
      label: "KD 近区降敏",
      desc: "近区阈值 / 近区倍率" },
    PID_INPUT_FILTER: { type: "multi", fields: [{ label: "开启", min: 0, max: 1 }, { label: "自动关闭阈值", min: 5, max: 100 }, { label: "自动恢复阈值", min: 5, max: 100 }, { label: "计数倍数(×0.1)", min: 1, max: 50 }],
      label: "PID 输入滤波",
      desc: "开关 / 自动关间隔阈值 / 自动开间隔阈值 / 变化后计数倍数；超出 N 周期临时关滤波" },
