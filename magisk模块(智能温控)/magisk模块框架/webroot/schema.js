...
window.B6X_SCHEMA = {
    ...
    // 不控制折叠；展开/折叠由 PERF_ENABLED 总开关驱动（master）。组内按模式切换显示参数面板
    {
      ...
      modePanels: [
        { when: "1", title: "PID 模式控制",
          keys: ["PID_KP", "PID_KI", "PID_KI_VAR", "PID_KD", "PID_KD_MEM",
                 "PID_INPUT_FILTER", "PID_ALPHA",
                 "PID_CPU_COMP", "PID_COLD"] },
  ...
  keys: {
    ...
    // ---- [3] PID ----
    PID_KP: { type: "multi", fields: [{ label: "近/远分界(0.1°C)", min: 10, max: 300 }, { label: "近区增益", min: 1, max: 1000 }, { label: "远区增益", min: 1, max: 1000 }],
      label: "KP 比例（分界 + 近区 + 远区）",
      desc: "偏差≥分界用远区斜率（300=偏差3°满幅）、<分界用近区低斜率，减小基准附近制冷抖动" },
    ...
    PID_KD: { type: "multi", fields: [{ label: "近/远分界(0.1°C)", min: 10, max: 300 }, { label: "近区增益", min: 1, max: 1000 }, { label: "远区增益", min: 0, max: 1000 }],
      label: "KD 微分（分界 + 近区 + 远区）",
      desc: "变化量≥分界用远区斜率、<分界用近区低增益，抑制小波动D抖动；每周期每度每KD为0.001归一化输出" },
    PID_KD_MEM: { type: "multi", fields: [{ label: "记忆衰减(×1000)", min: 1, max: 1000 }, { label: "记忆幅值基础(×1000)", min: 1, max: 1000 }, { label: "幅值斜率(×1000/°C)", min: 1, max: 1000 }],
      ...
      desc: "衰减：D记忆每周期向0收敛固定量；幅值基础：D记忆对称钳位基准值；斜率：距基准温度每°C上限增加量；实际上限=基础+距基准°C×斜率" },
