...
### 1. PID 模式下风扇转速远滞后于制冷强度/热面温度（2026-08-05 发现，未定位）

**现象**：PID 模式（CTRL_MODE=1）下，风扇转速目标长期显著低于制冷强度与热面温度所对应的需求，呈"制冷已拉满、热面已超温、风扇仍在中低转速"的状态，并非滤波/EMA 带来的轻微滞后。
...
**状态**：未定位根因。2026-08-05 已实施修正（PID/Gear 风扇目标改用限速后实际制冷重算），待真机验证后补记结论。
