...
> **归口（2026-09-22 固化）**：本文件是**已知缺陷及其状态与根因**的唯一归口处；**待做事项（动作）与真机验证清单**归 [`待办.md`](待办.md)。同一事项两边都有时，本文件留状态与根因，`待办.md` 只留待做动作并链接过来。
> 规则：
...
> **口径（本项状态的唯一有效源，2026-09-22 重申）**：`ensureUsableController()`（下发前校验）与 `checkCommandStall()`（锁死自愈）**均未根治，状态：未解决**——锁 125/4500 仍偶发。下方「已尝试的解决」是**已下发的措施**，不等于已修复；凡本仓库其他文档（含反编译分析类文档）出现与之矛盾的「已实施修复 / 已修复」表述，一律以本节为准。真机验证清单见 [`待办.md`](待办.md)（第四轮第 32~37 条一带）。
...
**状态**：**未验证**。上述任一条失败都会让"合并已可用"这个结论不成立，须逐条在真机上过。**逐项验法与判据见 [`待办.md`](待办.md) 的两处「需真机确认」节**（本节只留状态，不重复验法）。
...
4. **键盘弹起时底栏 inset 冲突**（根因：material 给底栏自带的 inset 监听把键盘高度写进底栏 `paddingBottom`，与 `ui/EdgeToEdge` 的高度反推公式叠加成递减）— **已在第四轮补丁轮修掉**，代码层面已改；**状态：已修待真机验证**，验法（反复拉起/收起键盘 5 次以上，底栏高度不再变化）见 [`待办.md`](待办.md) 第四轮第 47 条。原「本轮未修」的口径已作废。

**状态**：**未验证**。上述任一条都要在真机上过；其中第 3 条的误判代价最高（会把已部署好的进程连产物一起清掉）。**逐项验法与判据见 [`待办.md`](待办.md) 的「需真机确认（第四轮…）」节**（本节只留状态与根因，不重复验法）。
...
### 8. 四项已知缺陷与有意取舍（2026-09-20 记录；原 `待办.md` §已知未修，按归口迁入）
