...
### 2. 风扇日志表现为"单次突降 400+"，实为多周期跳过累积（2026-09-13 记录，机制已定位、未修）

**现象**：真机日志中相邻两条下发行之间的转速差可达 400~750，超过单周期限速步长，看起来像一次突降突破了限速。（来源：用户描述的真机日志；**本次未复核原始日志文件**，此处仅记录现象。）

**已确认机制**（代码级证据；`tempctrl.c` 行号会随源码变动漂移，故指法统一用**函数名 + 关键判据/格式串片段**，不写行号）：
1. 控制流程顺序为「先 `rate_limit_fan()`（推进内部累积量 `actual_rpm`）→ 再 `apply_gear_direct()`」：内部转速累积量**在就绪后每个控制周期都被推进**（`rate_limited_execute()` 的未就绪守卫提前返回时不推进，该期间不下发、也无日志），是否真正下发由 `apply_gear_direct()` 内部调用的 `should_skip_dispatch()` 决定。
2. `should_skip_dispatch()` 的跳过路径中（回传就绪分支 3 条 + 回传异常退化分支 1 条），**只有回传就绪分支里的纯制冷变化死区分支**（判据 `adiff < COLD_DEADZONE && !near_extreme`）**完全不看转速**——它只比较目标制冷与回传制冷，制冷落在死区内即跳过整次下发；**其余三条都在比较转速**（两条「已到位」判据 `send_rpm == cooler_rpm_real` 与 `cold == cooler_cold_real && send_rpm == cooler_rpm_real`；回传异常退化判据含 `windOC == last_rpm`），故它们只在转速（及制冷）确实未变时才跳过，本身**不累积**内部步进。
3. 常驻 RPM 日志行（`apply_gear_direct()` 内 `write_log("%d%+.1f° %s 冷%d 热%d° RPM%d", …)`）位于跳过判定**之后**：连续 N 个被跳过的周期里 `actual_rpm` 已走了 N 步，但**日志一条都不留** → 下一次实际下发时，与上一条日志相比即表现为「一次跳 N×步长」。
4. 默认 `DEBUG_ENABLED`=0 时连"跳过了"这一事实都看不见——上述前三条跳过路径的诊断（`skip 已到位（mode=0 仅比较风扇）…` / `skip 已到位：目标冷…` / `skip 制冷变化死区：…`）与 `apply_gear_direct` 的 `apply_gear_direct 跳过下发…` 共四条均走 `debug_log(debug_exec, …)` 门控；唯一例外是回传异常退化分支的 `下发去重 回传异常（…）`，它是常驻 `write_log`（但只在回传 <0 时才可能出现）。

**多口径 RPM 日志**（混着两行做差会得到"伪突降"）：

| 日志行（`tempctrl.c` 定位） | 打印变量 | 口径 |
|---|---|---|
| `apply_gear_direct()` 内 `… 冷%d 热%d° RPM%d`（常驻，仅实际下发时一条） | `send_rpm` | **下发目标值**（本周期限速取整后要下发的 RPM，非设备实际） |
| `reconnect_align()` 内 `重连 断联%d秒 回传冷%d RPM%d`（常驻，重连汇总行） | `cooler_rpm_real` | **设备回传实际值** |
| `try_align_actual()` 内 `回传就绪对齐 冷=%d rpm=%d`（常驻，启动/长断连对齐时） | `actual_rpm` | **内部累积值**（`rate_limit_fan` 每周期推进的内部量，对齐瞬间被赋为回传值） |
| `main()` 启动初始化内 `LSP 回传承载就绪 制冷=%d ratio=%.2f rpm=%d`（常驻，启动承载就绪） | `pid_align_rpm` | **对齐参考值**（由制冷比例 `pid_ratio_from_cold` 换算的参考 RPM，不参与风扇下发） |
| （旁证）`try_align_actual()` 内 `回传超时（>%ds）未就绪，用保守值起步 冷=%d rpm=%d` | `actual_rpm` | 对齐兜底：回传长期缺失时用 `fan_rpm_min` 起步 |

**观测盲区**：全文件没有任何一条日志打印**运行期**的实际限速步长与防抖阈值（相关日志受 `DEBUG_ENABLED` 门控，默认不输出），无法从日志判读"当前生效的限速到底是多少"。

**定案手段**（留待下次真机复现）：取日志中相邻两条下发行（即 `apply_gear_direct()` 的常驻 RPM 行）的**时间戳秒差**：控制周期约 5s，5s = 未跳过；10/15/20s = 跳过了 1/2/3 个周期。对该区间算 `Δ转速 ÷ (Δt/5 − 1)`：若稳定落在步长附近 → 确证为多周期累积，而非单步突破限速。

**状态**：**未修**。本次改动未调整跳过时序，也未新增日志。
